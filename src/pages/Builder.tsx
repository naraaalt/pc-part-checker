...
export default function Builder() {
    ...
    useEffect(() => {
        if (build.cpu && build.cooler && !build.cooler.sockets.includes(build.cpu.socket)) {
            setBuild(previous => ({
                ...previous,
                cooler: undefined
            }));
    ...
    useEffect(() => {
        if (build.motherboard && build.pcCase && !build.pcCase.supportedFormFactors.includes(build.motherboard.formFactor)) {
            setBuild(previous => ({
                ...previous,
                pcCase: undefined,
                caseFan: undefined
    ...
    useEffect(() => {
        if (build.pcCase && build.caseFan && !build.pcCase.fanSizes.includes(build.caseFan.size)) {
            setBuild(previous => ({
                ...previous,
                caseFan: undefined
            }));
    ...
    const filteredCases = build.motherboard
        ? pcCases.filter(c => c.supportedFormFactors.includes(build.motherboard!.formFactor))
        : pcCases;
    ...
    const hasRemainingM2 = build.motherboard ? build.motherboard.m2Slots - usedM2 > 0 : true;
    const hasRemainingSata = build.motherboard ? build.motherboard.sataPorts - usedSata > 0 : true;
    ...
    const storage2Disabled = !build.motherboard || (!hasRemainingM2 && !hasRemainingSata);
                    ...
                    <div className="component-row">
                        ...
                        {build.pcCase && (
                            <small className="component-text">
                                {build.pcCase.fanSizes.map(s => `${s}mm`).join(", ")} slots, max {build.pcCase.maxCaseFans} fans
                            </small>
                        )}
                        {!build.pcCase && (
                            <small className="component-text">
                                Select a PC Case first
                            </small>
                        )}
                        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                            ...
                            {build.caseFan && (
                                <input
                                    ...
                                    onChange={(e) => {
                                        const count = Math.max(1, Math.min(build.pcCase?.maxCaseFans ?? 12, parseInt(e.target.value) || 1));
                                        setBuild(previous => ({
                                            ...previous,
                                            caseFan: previous.caseFan ? { ...previous.caseFan, count } : undefined
                                        }));
                                    }}
                                />
                            )}
